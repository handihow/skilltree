...
const options = [
  {
    ...
    helpText:
      "Recommended! Use this option if you want to create accounts for all your students so they can interact with this skilltree",
  },
  {
    name: "names_only",
    title: "Full names only",
    helpText:
      "Use this option if you want to add students by name and let the system automatically assign usernames & passwords for their accounts. Afterwards you can send the login details to your students.",
  },
  {
    name: "emails_only",
    title: "Email addresses only",
    helpText:
      "Use this option if you want to create accounts for all your students without entering full names. Names will be added automatically when students log in with Google or Microsoft, or they can be updated by the student in their profile.",
  },
  {
    name: "ghost_accounts",
    title: "Ghost accounts",
    helpText:
      "Ghost accounts are an easy way to quickly generate accounts for your students. You can pass the created usernames/passwords to students and on first login they will be asked to update the account.",
  },
];

export class CompositionAddStudents extends Component<
...
> {
  constructor(props: RouteComponentProps<TParams>) {
    ...
    this.state = {
      ...
      option: "names_emails",
      ghostAccounts: 10,
      displayNames: RichTextEditor.createEmptyValue(),
      processedDisplayNames: [],
      emailAddresses: RichTextEditor.createEmptyValue(),
  ...
  autoCreateEmails = (number: number, displayNames?: string[]) => {
    ...
    for (let index = 0; index < number; index++) {
      ...
      if (typeof displayNames !== "undefined") {
        email =
          displayNames[index].toLowerCase().split(" ").join(".") +
          "@skilltree.student";
      }
      emails.push(email);
    }
  ...
  autoCreateDisplayNames = (number: number) => {
    ...
    for (let index = 0; index < number; index++) {
      const displayName =
        "Student " + (index < 9 ? "0" + (index + 1) : index + 1);
  ...
  incrementStep = () => {
    ...
    if (this.state.option === "ghost_accounts" && this.state.step === 1) {
      ...
      this.setState({
        processedDisplayNames: this.autoCreateDisplayNames(
          this.state.ghostAccounts
        ),
        processedEmailAddresses: this.autoCreateEmails(
          this.state.ghostAccounts
        ),
    ...
    if (
      (this.state.step === 1 && this.state.option === "emails_only") ||
  ...
  decrementStep = () => {
    let subtract = 1;
    if (
      (this.state.step === 3 && this.state.option === "emails_only") ||
      (this.state.step === 4 && this.state.option === "names_only")
    ) {
      subtract = 2;
    } else if (
      this.state.step === 4 &&
      this.state.option === "ghost_accounts"
    ) {
      subtract = 3;
    }
    this.setState({
  ...
  render() {
    return this.state.toMonitor ? (
      <Redirect
        to={`/compositions/${this.props.match.params.compositionId}/monitor`}
      />
    ) : (
      <React.Fragment>
        ...
        <article className="message is-primary">
          <div className="message-body">
            {this.state.step === 1 && (
              <React.Fragment>
                <div>Choose how you want to add users.</div>
              </React.Fragment>
            )}
            {this.state.step === 2 && (
              <React.Fragment>
                <div>
                  Enter the full names of your students in the input box below.
                </div>
                <div>Put each student on a new line.</div>
              </React.Fragment>
            )}
            {this.state.step === 3 && (
              <React.Fragment>
                <div>
                  Enter the email addresses of your students in the input box
                  below.
                </div>
            ...
            {this.state.step === 4 && (
              <React.Fragment>
                ...
                <div>
                  Download the list so you can share emails/passwords with your
                  students.
                </div>
                <div>
                  You must save the student list to create the accounts.
                </div>
          ...
          <div className="columns">
            <div className="column">
              <div className="field is-inline-block-desktop">
                <div className="field is-narrow">
                  <label className="label">Add student options</label>
            ...
            {this.state.option === "ghost_accounts" && (
              <div className="column is-3">
                <div className="field">
                  <label className="label" htmlFor="ghostAccounts">
                    Number of ghost accounts
                  </label>
                  <div className="control">
                    <input
                      className="input"
        ...
        {this.state.step === 1 && (
          <article className="message is-link">
            <div className="message-body">
              <React.Fragment>
                <div>
                  {options.find((o) => o.name === this.state.option)?.helpText}
                </div>
              </React.Fragment>
            </div>
          </article>
        ...
        <div className="level" style={{ marginTop: "20px" }}>
          <div className="level-left">
            {this.state.step > 1 && (
              <button className="button" onClick={this.decrementStep}>
                Back
              </button>
            )}
            {this.state.step === 1 && (
              <button className="button" onClick={this.backToMonitor}>
                Cancel
              </button>
            )}
          </div>
          <div className="level-right">
            ...
            {this.state.step === 4 && (
              <button
                className="button is-primary"
                onClick={() => this.addStudentList(false)}
              >
                Save student list
              </button>
            )}
        ...
        <div className={`modal ${this.state.showWarning ? "is-active" : ""}`}>
          ...
          <div className="modal-card">
            ...
            <section className="modal-card-body">
              ...
              <p>
                You will not have access to the automatically generated
                passwords after this step.
              </p>
            </section>
            <footer className="modal-card-foot">
              ...
              <button className="button" onClick={this.closeWarning}>
                Cancel
              </button>
            </footer>
          </div>
