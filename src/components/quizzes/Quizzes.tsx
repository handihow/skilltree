...
class Quizzes extends Component<IQuizzesProps, IQuizzesState> {
  ...
  async componentDidMount() {
    ...
    const unsubscribeOwned = db
      ...
      .onSnapshot((querySnapshot) => {
        const allHostedDomainQuizzes = querySnapshot.docs.map(
          (doc) => doc.data() as IQuiz
        );
        const ownQuizzes = allHostedDomainQuizzes.filter(
          (hdc) => hdc.user === this.props.user.uid
        );
        const hostedDomainQuizzes = allHostedDomainQuizzes.filter(
          (hdc) => hdc.user !== this.props.user.uid
        );
        this.setState({
          quizzes: ownQuizzes,
    ...
    const unsubscribeShared = db
      ...
      .orderBy("lastUpdate", "desc")
      .onSnapshot((querySnapshot) => {
        const sharedQuizzes = querySnapshot.docs.map(
          (doc) => doc.data() as IQuiz
        );
        this.setState({
  ...
  addQuiz = async (title, data) => {
    ...
    let standardFeedback =
      '{ "pages": [  {   "name": "page1",   "elements": [    {     "type": "boolean",     "name": "question1",     "title": "Completed"    },    {     "type": "comment",     "name": "question2",     "title": "Comment"    }   ],   "title": "Feedback"  } ], "showQuestionNumbers": "off"}';
  ...
  render() {
    ...
    return this.state.toBuilder ? (
      ...
      <React.Fragment>
            ...
            {this.props.user.hostedDomain && (
              ...
              >
                <a href="# " onClick={() => this.changeActiveTab("domain")}>
                  Quizzes in {this.props.user.hostedDomain}
                </a>
              </li>
            )}
          </ul>
            ...
            <article className="message is-primary">
              ...
              <div className="message-body">
                <div className="content">
                  <p>
                    You can add a new Quiz clicking on the button top right.
                  </p>
                </div>
              </div>
            </article>
        ...
        <div className={`modal ${this.state.isActive ? "is-active" : ""}`}>
          ...
          <div className="modal-card">
            ...
            <section className="modal-card-body">
              You are about to delete quiz '{this.state.currentQuiz?.title}'. Do
              you want to delete?
            </section>
        ...
        <div
          className={`modal ${this.state.isAddingOrEditing ? "is-active" : ""}`}
        >
          <div className="modal-background"></div>
          <div className="modal-card">
            <header className="modal-card-head">
              <p className="modal-card-title">
                {this.state.isEditingTitle ? "Edit quiz title" : "Add quiz"}
              </p>
              <button
                className="delete"
                aria-label="close"
            ...
            <AddQuiz
              ...
              updateQuizTitle={this.updateQuizTitle}
