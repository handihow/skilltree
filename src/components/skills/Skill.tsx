...
class Skill extends Component<ISkillProps, ISkillState> {
              ...
              .then(async (resultsSnap) => {
                if (!resultsSnap.empty) {
                  ...
                  for (let index = 0; index < results.length; index++) {
                    ...
                    if (
                      result.progress &&
                      result.progress[compositionId] &&
                      result.progress[compositionId] > 0
                    ) {
                      userResult.overallProgressOnComposition =
                        result.progress[compositionId];
  ...
  prepareDeleteLink = (id) => {
    if (
      !this.state.skill ||
      !this.state.skill.title ||
      !this.state.skill.links ||
      !this.state.skill.path
    )
      return;
    const linkIndex = this.state.skill.links.findIndex(
      (link) => link.id === id
    );
    if (linkIndex > -1) {
      this.setState({
  ...
  deleteLink = () => {
    ...
    const { dispatch } = this.props;
    dispatch(hideModal());
    const updatedLinks = this.state.skill.links.filter(
      (link) => link.id !== this.state.linkId
    );
    db.doc(this.state.skill.path)
      .update({
        links: updatedLinks,
      })
      .then((_) => {
  ...
  render() {
    return this.state.toSkillsList ? (
      <Redirect to={"/skills"} />
